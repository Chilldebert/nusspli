...
{*
			...
			{foreach item=row from=$stats.count_media}
                ...
                {assign var="data" value=$data|cat:"<slice title='$rowname' url='search.php?q="$rownameurl"%26fields=collection%26nowild=1'>$rowcount</slice>"}
            {/foreach}
			{assign var="data" value=$data|cat:"</pie>"}
...
*}
    ...
    <td class="center">
      <table width="100%">
        ...
        <tr>
          <td><h3>{$lang.videobycollection}:</h3></td>
          <td>
            <table class="collapse">
            {foreach item=row from=$stats.count_collection}
              <tr><td><a href='search.php?q="{$row.name|escape:url}"&amp;fields=collection&amp;nowild=1'>{$row.name}</a>:</td><td>{$row.count}</td></tr>
            {/foreach}
            </table>
          </td>
        </tr>
