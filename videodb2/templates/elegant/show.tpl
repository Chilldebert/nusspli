{*
  ...
  $Id: show.tpl,v 1.33 2011/02/11 07:43:39 andig2 Exp $
*}
...
<div id="actions">
    ...
    <div id="actionsbuttons">
        ...
        <form action="delete.php" name="deleteform">
            <input type="hidden" name="id" value="{$video.id}"/>
            <input type="submit" class="button" value="{$lang.del}" onclick="return(confirm('{if $video.title}{$video.title|escape:javascript|escape}: {/if}{$lang.really_del|escape:javascript|escape}?'))" />
        </form>
...
<tr >
    ...
    <td>
      <table>
        ...
        {/if}
        {if $video.collection}
            <tr><td><h3>{$lang.collection}:</h3></td><td><a href="search.php?q=%22{$video.collection|escape:url}%22&amp;fields=collection&amp;nowild=1">{$video.collection}</a></td></tr>
        {/if}
        {if $video.owner}
            <tr>
