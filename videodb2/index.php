 ...
 * @version $Id: index.php,v 2.98 2010/04/04 10:36:38 andig2 Exp $
 */
...
function get_collection_sql($m)
{
    switch ($m) {
        case -2:    
            ...
            break;
        case -1:    
            $collection  = 'collection != '.MEDIA_WISHLIST;
            break;
        default:    
...
session_default('collectionfilter', -1);
...
$SQL    = 'SELECT '.TBL_DATA.'.id, '.TBL_DATA.'.diskid, 
                  ...
                  lastupdate, mediatype, collection,
...
smarty_display('list.tpl', get_current_user_id().'|'.$filter.$pageno);
