 ...
 * @version $Id: search.php,v 2.56 2010/04/04 10:33:26 andig2 Exp $
 */
...
$replace_fields = array('mediatype' => TBL_MEDIATYPES.'.name',
                        'collection' => TBL_COLLECTION.'.name',
...
{
    // name search?
    $fields = ($isname) ? array('director', 'actors') : array_keys($search_fields);
}
...
if (isset($q))
{
    ...
    $select = 'SELECT DISTINCT '.TBL_DATA.'.id, '.TBL_DATA.'.diskid,
                      ...
                      lastupdate, mediatype, collection, created,
			...
			LEFT JOIN '.TBL_COLLECTION.' ON '.TBL_DATA.'.collection = '.TBL_COLLECTION.'.id
