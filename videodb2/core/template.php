 ...
 * @version $Id: template.php,v 1.65 2010/11/05 10:38:47 andig2 Exp $
 */
...
function tpl_filters($filter,$showtv)
{
    ...
    $smarty->assign('mediatype',  session_get('mediafilter'));
	
	// create collection selectbox
    $smarty->assign('collectionfilter', out_collection(array(-2 => $lang['filter_any'], -1 => $lang['filter_available'])));
    $smarty->assign('collection',  session_get('collectionfilter'));
...
function prepare_cast($cast)
{
    ...
    // reformat roles
    $actors = preg_replace('/\(/', '<small>(', $actors);
    $actors = preg_replace('/\)/', ')</small>', $actors);
...
function tpl_edit($video)
{
    ...
    // create collection selectbox
    $smarty->assign('collection', out_collection());
    if (!isset($video['collection'])) $video['collection'] = $config['collectiondefault'];
