 ...
 * @version $Id: show.php,v 2.50 2010/11/15 09:29:45 andig2 Exp $
 */
...
if (empty($id)) 
{
    ...
    if ($config['multiuser'])
    {
        ...
        if ($_SESSION['vdb']['owner'] && 
           ($_SESSION['vdb']['owner'] == $all || 
            check_permission(PERM_READ, $uid = get_userid($_SESSION['vdb']['owner'])))) 
        {
            if ($_SESSION['vdb']['owner'] == $all) $uid = -1; 
            $owner_id = $uid;
        } 
        ...
        if ($owner_id && $owner_id >= 0) $_SESSION['vdb']['owner'] = get_username($owner_id);
        else $_SESSION['vdb']['owner'] = $all;
...
if (!empty($id))
{
    ...
    $SELECT = 'SELECT '.TBL_DATA.'.id, title, subtitle, language, diskid, comment,
                      ...
                      email, rating, custom1, custom2, custom3, custom4, collection,
                      !ISNULL('.TBL_USERSEEN.'.video_id) AS seen,
                      '.TBL_USERS.'.name AS owner, '.TBL_MEDIATYPES.'.name AS mediatype  , '.TBL_COLLECTION.'.name AS collection
                 FROM '.TBL_DATA.'   
			...
			LEFT JOIN '.TBL_COLLECTION.' ON collection = '.TBL_COLLECTION.'.id
